...
public class OldElevator {
    private static OldElevator m_singleton = null;
	...
	public static OldElevator getInstance() {
		if (m_singleton == null) {
			m_singleton = new OldElevator();
		}
		return m_singleton;
	}

    public void extendElevatorHigh() {
        if ((m_elevatorExtendWinch.getSelectedSensorPosition() < kHighTargetPoint) && !hasInput) {
            output = (kHighTargetPoint - m_elevatorExtendWinch.getSelectedSensorPosition()) * kExtendingP;

            elevatorState = "extending";
            m_elevatorExtendWinch.set(output);
            m_elevatorRetractWinch.set(output * kPullyRatio);
        } else if (m_elevatorExtendWinch.getSelectedSensorPosition() >= kHighTargetPoint) {
    ...
    public void extendElevatorMiddle() {
        if ((m_elevatorExtendWinch.getSelectedSensorPosition() < kMidTargetPoint) && !hasInput) {
            output = (kMidTargetPoint - m_elevatorExtendWinch.getSelectedSensorPosition()) * kExtendingP;

            elevatorState = "extending";
            m_elevatorExtendWinch.set(output);
            m_elevatorRetractWinch.set(output * kPullyRatio);
        } else if (m_elevatorExtendWinch.getSelectedSensorPosition() >= kMidTargetPoint) {
            elevatorState = "extended";
            m_elevatorExtendWinch.set(0);
            m_elevatorRetractWinch.set(0);
        }
    ...
    public void extendElevatorLow() {
        if ((m_elevatorExtendWinch.getSelectedSensorPosition() < kLowTargetPoint) && !hasInput) {
            output = (kLowTargetPoint - m_elevatorExtendWinch.getSelectedSensorPosition()) * kExtendingP;

            elevatorState = "extending";
            m_elevatorExtendWinch.set(output);
            m_elevatorRetractWinch.set(output * kPullyRatio);
        } else if (m_elevatorExtendWinch.getSelectedSensorPosition() >= kLowTargetPoint) {
            elevatorState = "extended";
            m_elevatorExtendWinch.set(0);
            m_elevatorRetractWinch.set(0);
        }
    ...
    public void elevatorPeriodic() {
        ...
        if (isScoring && goingHigh) {
            ...
            if ((m_elevatorExtendWinch.getSelectedSensorPosition() < kHighTargetPoint) && !hasInput) {
                error = kHighTargetPoint - m_elevatorExtendWinch.getSelectedSensorPosition();
